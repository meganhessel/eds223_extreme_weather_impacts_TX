...
#income_texas <- vect("data/ACS_2019_5YR_TRACT_48_TEXAS.gdb", layer = "X19_INCOME")
...
# Need to change geom_texas and income_texas to match the others 
```
...
tm_shape(diff_crop_epsg) + 
  tm_polygons(col = "firebrick4") + 
  tm_shape(roads_union) + 
  tm_lines() +
  tm_basemap("CartoDB.PositronNoLabels") +
  tm_title(text = "All Backouts in Houston, TX")
```
...
plot(feb7sf) # bins: 0, 1000, 1500, 3000, 4000, 5000
plot(feb16sf) # bins: 0, 5000, 10000, 15000, 20000, 25000 


#tm_shape(feb7sf) + 
  #tm_raster(breaks = c(0, 1000, 1500, 3000, 4000, 5000))

#tm_shape(feb16sf) + 
  #tm_raster(breaks = c(0, 5000, 10000, 15000, 20000, 25000 ))
...
#blackouts_200m_highway <- st_difference(diff_mask_vec , road_buffer)
...
-  income data = income_texas 
-  texas geometeries = geom_texas

Pull layers 1 (geom) and 2 (income) out of the data - did when pulling out the data 
Left Join layers 1 and 2 on GEOID 
Manipulation - CRS check / transform 
st_join with blackout house data 
data wrangle to summarise means income by census tract 

Join Texas geometries and income data layers 
```{r}
# Left join geom and incomes 
texas_df <- left_join(geom_texas, income_texas, by = 'GEOID')
...
```{r}
# Check CRS 
crs_check(texas_df, home_blackout)
